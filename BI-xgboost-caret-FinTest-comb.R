...
library("knitr")
library("RSQLite")
Sys.setenv(JAVA_HOME='C:\\Program Files (x86)\\Java\\jre1.8.0_60')
library("rJava")
...
#tuning parameters with CV in caret (50 features)
ctrl <- trainControl(method="cv", repeats=1, number=10)
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ ., data=top50wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
sum(xgb.predict == finTest$Activity)/length(xgb.predict)
# predicting on validation dataset (50 features)
xgb.predict <- predict(xgb.tune, newdata=Top50.ft) 
sum(xgb.predict == Top50.ft$Activity)/length(xgb.predict)


#tuning parameters with CV in caret (100 features)
ctrl <- trainControl(method="cv", repeats=1, number=10)
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ ., data=top100wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
#tuning parameters with CV in caret (150 features_)
ctrl <- trainControl(method="cv", repeats=1, number=10)
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ ., data=top150wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ ., data=top200wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ ., data=top250wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ ., data=Top100comb.wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
xgb.predict <- predict(xgb.tune, newdata=Top100comb.ftd) 
sum(xgb.predict == Top100comb.ftd$Activity)/length(xgb.predict)
...
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ ., data=Top200comb.wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
xgb.predict <- predict(xgb.tune, newdata=Top200comb.ftd) 
sum(xgb.predict == Top200comb.ftd$Activity)/length(xgb.predict)
...
grid <- expand.grid(nrounds=c(500, 100),
                    max_depth= c(10, 15, 20),
                    eta = 0.3)
xgb.tune<- train(Activity ~ PC3 + PC7 + PC6 + PC50 + PC19 + PC26 + 
                   ...
                   Dim.55 + PC262 + Dim.312, data=Top500comb.wd,
                 method="xgbTree",
                 trControl=ctrl,
                 tuneGrid = grid)
xgb.tune$bestTune
...
xgb.predict <- predict(xgb.tune, newdata=Top500comb.ftd) 
sum(xgb.predict == Top500comb.ftd$Activity)/length(xgb.predict)
