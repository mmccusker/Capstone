...
#setwd("~/Capstone/Step_1_all_classifiers")
getwd()
library("knitr")
library("RSQLite")
...
grid <- expand.grid(sigma=c(0.03, 0.05, 0.07),
                    C= 1)
svm.tune<- train(Activity ~ ., data=WD,
                 method="svmRadial",
                 trControl=ctrl,
                 tuneGrid = grid)
svm.tune$bestTune
...
grid <- expand.grid(sigma=c(0.03, 0.05, 0.07),
                    C= 1)
svm.tune<- train(Activity ~ ., data=top50wd,
                 method="svmRadial",
                 trControl=ctrl,
                 tuneGrid = grid)
svm.tune$bestTune
...
grid <- expand.grid(sigma=c(0.03, 0.05, 0.07),
                    C= 1)
svm.tune<- train(Activity ~ ., data=top100wd,
                 method="svmRadial",
                 trControl=ctrl,
                 tuneGrid = grid)
svm.tune$bestTune
...
#tuning parameters with CV in caret (150 features)
ctrl <- trainControl(method="cv", repeats=1, number=10)
grid <- expand.grid(sigma=c(0.03, 0.05, 0.07),
                    C= 1)
svm.tune<- train(Activity ~ ., data=top150wd,
                 method="svmRadial",
                 trControl=ctrl,
                 tuneGrid = grid)
svm.tune$bestTune
...
ctrl <- trainControl(method="cv", repeats=1, number=10)
grid <- expand.grid(sigma=c(0.01, 0.03, 0.05, 0.07),
                    C= 1)
...
#tuning parameters with CV in caret (AIC on top 500 features)
ctrl <- trainControl(method="cv", repeats=1, number=10)
...
grid <- expand.grid(sigma=c(0.01, 0.03, 0.05),
                    C= 1)
svm.tune<- train(Activity ~ ., data=Top100comb.wd,
                 method="svmRadial",
                 trControl=ctrl,
                 tuneGrid = grid)
svm.tune$bestTune
...
svm.predict <- predict(svm.tune, Top100comb.ftd) 
sum(svm.predict == Top100comb.ftd$Activity)/length(svm.predict)
...
grid <- expand.grid(sigma=c(0.01, 0.03, 0.05),
                    C= 1)
svm.tune<- train(Activity ~ ., data=Top200comb.wd,
                 method="svmRadial",
                 trControl=ctrl,
                 tuneGrid = grid)
svm.tune$bestTune
...
svm.predict <- predict(svm.tune, Top200comb.ftd) 
sum(svm.predict == Top200comb.ftd$Activity)/length(svm.predict)
...
grid <- expand.grid(sigma=c(0.01, 0.03, 0.05),
                    C= 1)
svm.tune<- train(Activity ~ PC3 + PC7 + PC6 + PC50 + PC19 + PC26 + 
...
svm.predict <- predict(svm.tune, Top500comb.ftd) 
sum(svm.predict == Top500comb.ftd$Activity)/length(svm.predict)
